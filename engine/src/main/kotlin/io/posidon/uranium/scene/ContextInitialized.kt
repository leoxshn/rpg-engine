...
abstract class ContextInitialized <SELF : ContextInitialized<SELF>> {
    ...
    fun objectShader(fragmentPath: String): Lazy<Shader> = onInit { context.loadObjectShader(log, fragmentPath) }
    fun screenShader(fragmentPath: String): Lazy<Shader> = onInit { context.loadScreenShader(log, fragmentPath) }
    fun customShader(vertexPath: String, fragmentPath: String): Lazy<Shader> = onInit { context.loadShader(log, fragmentPath, vertexPath) }
    fun ttf(path: String): Lazy<Font> = onInit { context.loadTTF(log, path) }
    fun texture(path: String): Lazy<Texture> = onInit { context.loadTexture(log, path) }
