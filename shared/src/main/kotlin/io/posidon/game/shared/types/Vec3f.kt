...
data class Vec3f(var x: Float, var y: Float, var z: Float) {
    ...
    companion object {
        fun mix(v1: Vec3f, v2: Vec3f, ratio: Float): Vec3f {
            val inverseRation = 1f - ratio
            val x = v1.x * inverseRation + v2.x * ratio
            val y = v1.y * inverseRation + v2.y * ratio
            val z = v1.z * inverseRation + v2.z * ratio
            return Vec3f(x, y, z)
        }
    ...
    fun selfMix(other: Vec3f, ratio: Float) {
        val inverseRation = 1f - ratio
        x = x * inverseRation + other.x * ratio
        y = y * inverseRation + other.y * ratio
        z = z * inverseRation + other.z * ratio
    }
}