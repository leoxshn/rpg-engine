...
uniform float break_progress = 0.0;

out vec4 out_Color;

void main () {
    float center = 1.0 - 2 * length(uv - 0.5);
    float value = center * (1 - break_progress) + max(open_simplex_bcc_base(vec3(uv * (3.0 + break_progress * 2.0) + noise_offset, millis * 0.0003)).x, center - 1.0) * mix(center * 0.2, center * 1.2, break_progress);
    out_Color = vec4(value, center * break_progress, 0.0, value);
}